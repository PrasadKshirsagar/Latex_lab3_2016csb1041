...
%DIF DEL old.tex   Mon Feb 19 08:58:20 2018
%DIF ADD new.tex   Mon Feb 19 15:39:16 2018
...
% 1st. author
\alignauthor
\DIFdelbegin \DIFdel{Mukesh Saini}\DIFdelend \DIFaddbegin \DIFadd{Prasad Kshirsagar}\DIFaddend \\
       \affaddr{Dept. of Computer Science}\\
       \affaddr{National University of Singapore}\\
       \email{mksaini@comp.nus.edu.sg}
% 2nd. author
\alignauthor
\DIFdelbegin \DIFdel{Raghudeep Gadde}\DIFdelend \DIFaddbegin \DIFadd{Komal Chugh}\DIFaddend \\
       \affaddr{Dept. of ECE}\\
       \affaddr{National University of Singapore}\\
...
% 4th. author
\alignauthor \DIFdelbegin \DIFdel{Shuicheng Yan}\DIFdelend \DIFaddbegin \DIFadd{Ram Krishna}\DIFaddend \\
       \affaddr{Dept. of ECE}\\
       \affaddr{National University of Singapore}\\
       \email{eleyans@nus.edu.sg}
% 5th. author
\alignauthor \DIFdelbegin \DIFdel{Wei Tsang Ooi}\DIFdelend \DIFaddbegin \DIFadd{Vivek Verma}\DIFaddend \\
       \affaddr{Dept. of Computer Science}\\
       \affaddr{National University of Singapore}\\
...
\DIFaddbegin \DIFadd{Changing Statement :
...
dividual components.\DIFaddbegin \DIFadd{Changing Statement :
}\DIFaddend \subsection{Design Principles}
...
patch(x{'},y{'}) is greater than threshold}\\
    ...
    \end{cases}
\end{equation}
The 1\textquotesingle s in the patch image shows potentially occluded re-
gions.
...
\subsubsection{Tilt}
\par In this work, we define tilt as the rotation of the camera around
...
steps are taken to calculation tilt: \DIFaddbegin \DIFadd{Changing Statement :}\DIFaddend \\
\begin{figure}[htp]
...
\par Shakiness is calculated based on the method described in [4].
...
for any value above 300, S s is saturated to 1.\par
...
and their corresponding lengths.
 \end{itemize} 
\begin{equation}
\begin{pmatrix} 0 & 0.4 & 0.2 & 0.1 & 0.2 & 0.1\\ 0.6 & 0 & 0.1 & 0.1 & 0.1 & 0.1\\0.5 & 0.1 & 0 & 0.1 & 0.2 & 0.1\\ 0.2 & 0.2 & 0.4 & 0 & 0.1 & 0.1\\ \end{pmatrix}
\end{equation}
\begin{equation}
\begin{pmatrix} 1/31 & 2/31 & 4/31 & 7/31 & 7/31 & 6/31 & 4/31\\ 3/12 & 4/12 & 2/12 & 1/12 & 1/12 & 1/12 & 0\\ 2/15 & 3/15 & 4/15 & 3/15 & 2/15 & 1/15 & 0\\ 1/31 & 2/31 & 4/31 & 7/31 & 7/31 & 6/31 & 0 \end{pmatrix}\end{equation}
\par We use affine transformation to classify the video, giving an accuracy of $\approx $77\% on our dataset. However, since learning is one
...
\par We can have different quality videos because of the limitation of
...
while a value of 0 means at least 25\% \text{pixels are burnt.}
 \end{itemize} 
\par The individual quality scores are multiplied to calculate overall
video quality score $S^q$ , i.e.,
\begin{equation}
    S^q = S^b \times S^{br} \times S^{im} \times S^c \times S^{bp}
\end{equation}
\par We have chosen to multiply the individual scores because we
want to give priority to the videos that are good in all aspects.
\subsection{Diversity}
\par The aspect of diversity is included in the framework by calculat-
...
tion.
    \item $\bigtriangledown$ - The time for which the particular camera is selected. It
...
\end{equation}
\par where n is number of cameras, and v ij is the view similarity
...
\end{equation}
where $I_{i}^c$ is the current frame of $i^{th}$ camera, $I_{j}^h$ is the $j^{th}$
...
2) video gets selected until Cam 1\textquotesingle s diversity increases enough so
...
\par For all the videos of the selected class, we have three metrics:
...
\end{equation}
where $\alpha_{1}$ ,  $\alpha_{2}$ , and  $\alpha_{3}$ are weighting coefficients and  $\alpha_{1}$ +  $\alpha_{2}$ +
 $\alpha_{3}$ = 1. In the experiments, we are using  $\alpha_{1}$ =  $\alpha_{2}$ =  $\alpha_{3}$ ,which gives equal weightage to the quality, diversity, and stabil-
...
We therefore need to keep  $\alpha_{3}$ higher in this case. Furthermore, if
the videos are generally bad in the quality, we can give set high
value for  $\alpha_{1}$ . The shot from the video with the highest score is
selected at the current time instant.
\subsection{Length Selection}
\par To determine the switching instant, we are using a method which
...
Suppose $S_{1}^f$ is the final combined score of the best camera and $S_{2}^f$
of the second best camera. Now the length for the currently selected
video, $L^s$ , is determined as follows:\par
\begin{equation}
    L^s = (1 - \varsigma)L^e + \varsigma L^b v
\end{equation}
where $\nu$ is the difference of the scores, i.e., $\nu$ =
and ς is
weighting coefficient. In our experiments, we found the empirical
values of $L^b$ = 25 and $\zeta$ = 0.1 works well. A higher value of
...
\section{EVALUATION}
\par The main goal of the experiments is to show that the proposed
...
mashup.
\subsection{Dataset}
\par The main application of the proposed framework is to combine
...
\par In the user study, we ask the users to watch the mashups created
...
\par The average responses of the users, along with their standard
...
\par We observed that majority of users responded homogeneously
...
in comparison with VQ-Only based mashup.\par
...
\par User ratings for overall video quality are shown in the Figure 8.
...
\par The MoViMash framework takes automatic mashup creation meth-
...
\par In this paper, we have proposed and validated an online video
